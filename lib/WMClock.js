...
//}@dev
    ...
    this._suspend = { yes: false, begin: 0, end: 0 };
    ...
    function _onsuspend(pageHide) {
        if (pageHide) {
            that._suspend = { yes: true, begin: Date.now(), end: 0 };
            that["stop"]();
        } else {
            that._suspend.end = Date.now();
            that["start"]();
        }
    }
}
...
function _enterFrame(timeStamp) { // @arg DOMHighResTimeStamp|undefined - requestAnimationFrame give us timeStamp.
    ...
    // reduce the suspend time
    if (this._suspend.yes) {
        timeStamp -= this._suspend.end - this._suspend.begin;
        this._suspend = { yes: false, begin: 0, end: 0 };
    }
