...
function WMClock(options) { // @arg Object = {} - { vsync, speed }
                            ...
                            // @options.autoSuspend Boolean = false - pagevisibilityevent pause/resume
                            // @desc Master Clock.
//{@dev
    $valid($type(options, "Object|omit"), WMClock, "options");
    $valid($keys(options, "vsync|speed|autoSuspend"), WMClock, "options");
    if (options) {
        $valid($type(options.vsync, "Boolean|omit"), WMClock, "options.vsync");
        $valid($type(options.speed, "Number|omit"),  WMClock, "options.speed");
        $valid($type(options.autoSuspend, "Boolean|omit"), WMClock, "options.autoSuspend");
    }
//}@dev
    ...
    if (options["autoSuspend"]) {
        PageVisibilityEvent.on(_onsuspend);
    }
